...
class CineStreamApp {
    ...
    setupSearchPage() {
        ...
        if (searchInput && searchResults) {
            searchInput.addEventListener('input', this.debounce(async (e) => {
                ...
                if (query.length < 3) {
                    searchResults.innerHTML = '<p class="search-placeholder">Ingresa al menos 3 caracteres para buscar</p>';
                    return;
                }
    ...
    displaySearchResults(results) {
        ...
        if (filteredResults.length === 0) {
            container.innerHTML = '<p class="no-results">No se encontraron películas o series</p>';
            return;
        }
    ...
    loadFavoritesPage() {
        ...
        if (container) {
            ...
            if (favorites.length === 0) {
                container.innerHTML = '<p class="no-favorites">No tienes favoritos aún</p>';
                return;
            }
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.info-btn') || e.target.closest('.btn-secondary')) {
                const button = e.target.closest('.info-btn') || e.target.closest('.btn-secondary');
